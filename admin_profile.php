...
$loginHistoryStmt = $conn->prepare("SELECT action, ip_address, created_at FROM audit_log WHERE actor_username = ? AND action IN ('login', 'login_failed') ORDER BY created_at DESC LIMIT 8");
$loginHistoryStmt->bind_param("s", $user['username']);
...
<main class="page">
    <div class="page-head">
        <div>
            ...
            <div class="muted-copy">Manage your photo, contact details, and password for this admin account.</div>
        </div>
    ...
    <section class="panel panel-pad security-panel">
        <div class="security-head">
            <div>
                <h2 class="panel-title" style="border-bottom:0;margin-bottom:2px;padding-bottom:0"><i class="bi bi-shield-lock-fill"></i> Security &amp; Login Activity</h2>
                <div class="muted-copy">Recent sign-in attempts for this admin account.</div>
            </div>
            <button type="button" class="action-btn secondary-btn" id="changePasswordBtn"><i class="bi bi-key-fill"></i> Change Password</button>
        </div>
        <?php if(empty($loginHistory)): ?>
        <div class="security-empty">No login activity recorded yet.</div>
        <?php else: ?>
            ...
            <table class="login-table">
                ...
                <tbody>
                    <?php foreach($loginHistory as $entry):
                        $isSuccess = $entry['action'] === 'login';
                        $whenLabel = date("M j, Y g:i A", strtotime($entry['created_at']));
                        $ip = $entry['ip_address'] !== '' ? $entry['ip_address'] : '&mdash;';
                    ?>
                    <tr>
                        <td><?php echo htmlspecialchars($whenLabel, ENT_QUOTES); ?></td>
                        <td>
                            <?php if($isSuccess): ?>
                            <span class="login-result-chip login-success"><i class="bi bi-check-circle-fill"></i> Success</span>
                            <?php else: ?>
                            <span class="login-result-chip login-failed"><i class="bi bi-x-circle-fill"></i> Failed</span>
                            <?php endif; ?>
                        </td>
                        <td><?php echo $ip; ?></td>
                    </tr>
